'''program for remote control ev3 bot to go through a racetrack
the robot makes sure that all of the color checkpoints are passed'''
import time
...
def detectGate(color, gatecolor):
    '''compares the current color with the gatecolor
    if their components are all close return True'''
    matches = 0
    for i in range(3):
...
Sound.speak('press touch sensor when done calibration').wait()
while not ts.value():
    rc.process()
    home = turn.position
Sound.speak('calibrated').wait()
...
Sound.speak('press any remote control button to begin').wait()
while not rc.any():
    pass
